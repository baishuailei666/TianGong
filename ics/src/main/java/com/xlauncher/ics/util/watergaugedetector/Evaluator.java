package com.xlauncher.ics.util.watergaugedetector;
...
import static com.xlauncher.ics.util.watergaugedetector.Classifier.object_image;
...
public class Evaluator {
    ...
    private Evaluator(String detectorModelPath, String predictorModelPath, String classifierModelPath){
     ...
     * @return evaluator
     */
    public static Evaluator getInstance (String detectorModelPath, String predictorModelPath, String classifierModelPath){
        if (null == evaluator) {
            synchronized (Evaluator.class) {
                if (null == evaluator) {
                    ...
                    if (!detectorModelFile.exists() || !predictorModelFile.exists() || !classifierModelFile.exists()){
                        logger.error("模型文件不存在！");
                        return null;
                    }
                    evaluator = new Evaluator(detectorModelPath, predictorModelPath, classifierModelPath);
                    //保证模型只加载一次
    ...
    private static boolean loadModel(){
        if (null == detector || null == predictor || classifier == null) {
...
//        System.out.print(object_class[0]);
        ...
        if (object_class[0] == 1) {
            int[][] prediction_output = predictor.predict(object_image);
